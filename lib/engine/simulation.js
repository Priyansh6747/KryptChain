  ...
  injectAttack(attackType, config = {}) {
    ...
    switch (attackType) {
      case 'double_spend': {
        ...
        targetTxId = tx.id;
        // Now go selfish — private chain will exclude this tx
        attackerNode.behavior = 'selfish';
        gs.emitEvent('ATTACK_INJECTED', attackerNode.id, {
          attackType,
          description: `⚡ Double-spend injected! Attacker (${attackerNode.id}) controls ${(fraction*100).toFixed(0)}% hash power. Chain is ${chainDepth} blocks deep — survival probability: ${chainDepth > 6 ? 'HIGH' : chainDepth > 3 ? 'MEDIUM' : 'LOW'}.`,
        });
        break;
      }
      case 'selfish_mining': {
